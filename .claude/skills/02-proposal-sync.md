---
name: proposal-sync
description: 提案階段 - 團隊內部資訊同步。彙整訪談結果，召集關鍵成員評估可行性，產出內部簡報摘要並通知相關人員。
---
...
使用 Write 工具建立本地檔案 `02_內部評估摘要_{日期}.md`，內容包含：
1. 客戶需求摘要（來自訪談紀錄）
...
使用 `glab` 更新來自 01 的 GitLab Issue：
...
> **Timeout 機制**：截止日到期若尚未收到關鍵角色回覆，發送催辦通知（Slack @mention 或 email）並告知 PM。
...
3. 使用 `glab` 在 GitLab Issue 加入 label `status:rejected` 並關閉 issue
