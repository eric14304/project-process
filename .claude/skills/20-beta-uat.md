...
3. **產出 SIT 驗收文件**（命名：`{客戶}_{專案}_SIT驗收文件_v1_{日期}.docx`），記錄：
   - Demo 網址
   - 本批次驗收功能範圍清單
   - 測試結果（通過 / 待修正）
...
2. **執行掃描**，產出掃描報告（含 Security Hotspots 清單）
3. **逐項確認並修正**：區分需修正（High/Critical）vs 可接受（Low/Medium with justification）
4. **產出「弱掃調整報告」**：
   - 每個 Hotspot 列出：類別、優先級、處理結果（已修正 / 可接受原因）
5. 客戶 IT/資安審查調整報告後放行，才進入最終驗收
